...
class MPD
  ...
  # if successful, otherwise an error will be raised
    ...
    # Raises a RuntimeError if the command failed
    #
    # If recursive is true (default), returns all subdirectories,
    # or else returns only direct children
    def directories( path = nil, recursive=true )
        response = ''
        if recursive
            if not path.nil?
    ...
    # If recursive is true (default), returns all subdirectories,
    # or else returns only direct children
    def files( path = nil, recursive=true )
        response = '' 
        if recursive
            if not path.nil?
                response = send_command "listall \"#{path}\""
    ...
    # If recursive is true (default), returns all subdirectories,
    # or else returns only direct children
    def songs( path = nil, recursive=true )
        response = ''
        if recursive
            if not path.nil?
    ...
    def build_songs_list( string )
        ...
        list << song
        #For some reason this is picking up directories some times, take
        #them out
        return list.select{|n| not n['directory'] }
    end
